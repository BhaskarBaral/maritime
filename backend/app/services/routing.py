...
from backend.app.services.ml_models import _latest_commodity_snapshot
...
def _berth_matches(berth_id: str, default_berth_label: str) -> bool:
    """
    True iff `berth_id` (e.g. "Berth 1", from berths.csv) refers to the same
    physical berth as `default_berth_label` (e.g. "Berth 10 (OJ 1)", from
    COMMODITY_ROUTING_MAP). Compares berth numbers, not raw substrings —
    a plain `"berth 1" in "berth 10"` substring check is a false positive
    (also true for "berth 1" in "berth 11"..."berth 17"), which used to
    misattribute every double-digit berth's commodity onto Berth 1.
    """
    num_a = re.search(r"\d+", berth_id)
    num_b = re.search(r"\d+", default_berth_label)
    if num_a and num_b:
        return num_a.group() == num_b.group()
    # Non-numeric ids (e.g. "SPM") — compare the alphabetic token directly.
    return berth_id.strip().lower() in default_berth_label.strip().lower()
...
def get_all_facilities_list() -> List[Dict[str, Any]]:
    ...
    for r in records:
        ...
        for comm, meta in COMMODITY_ROUTING_MAP.items():
            if any(_berth_matches(b_id, def_b) for def_b in meta["default_berths"]):
...
def recommend_cargo_route(
    ...
    for idx, row in df_fac.iterrows():
        ...
        if b_draft >= req_draft and (b_dwt >= req_dwt or b_dwt == 0):
            ...
            is_preferred = any(_berth_matches(b_id, pref) for pref in meta["default_berths"])
...
def get_live_berth_status() -> List[Dict[str, Any]]:
    """
    Real, data-grounded berth status for the 17 official NMPA berths.

    There is no live vessel/AIS occupancy feed in this project (see
    PROJECT_DOCUMENTATION/21_DATA_AUDIT_REAL_VS_SYNTHETIC.md), so "occupied"
    here does NOT mean a ship is physically moored right now — it means the
    berth's best-matched commodity actually moved real cargo in the latest
    recorded month of data/port_cargo_monthly.csv. Utilization is the real
    monthly tonnage for that commodity, annualised and checked against the
    berth's real capacity_mmt from data/berth_capacity.csv — the same
    calculation recommend_cargo_route() uses for a single commodity, applied
    here across all 17 berths at once.
    """
    df_fac = load_nmpa_facilities()
    try:
        snapshot = _latest_commodity_snapshot().set_index("commodity")["traffic_tonnes_current"]
    except Exception:
    ...
    for _, row in df_fac.iterrows():
        b_id = str(row.get("berth_id", ""))
        cap_mmt = float(row.get("capacity_mmt", 3.0) or 3.0)

        # Commodities this berth is rated for, ranked by real latest-month tonnage
        candidates = [
            comm for comm, meta in COMMODITY_ROUTING_MAP.items()
            if any(_berth_matches(b_id, def_b) for def_b in meta["default_berths"])
        ]
        best_commodity, best_tonnes = None, 0.0
        for comm in candidates:
            tonnes = float(snapshot.get(comm, 0.0))
            if tonnes > best_tonnes:
                best_commodity, best_tonnes = comm, tonnes

        monthly_mmt = (best_tonnes * 12) / 1_000_000.0
        utilization_pct = round(min(100.0, (monthly_mmt / max(cap_mmt, 0.5)) * 100), 1) if best_tonnes > 0 else 0.0

        statuses.append({
            "berth_id": b_id,
            "name": str(row.get("type_of_berth", b_id)),
            "occupied": best_tonnes > 0,
            "commodity": best_commodity,
            "capacity_mt": round(cap_mmt * 1_000_000),
            "max_draught_m": row.get("max_draught_m"),
            "max_dwt": row.get("max_dwt"),
            "hours_remaining": None,
            "crane_count": None,
            "utilization_pct": utilization_pct,
            "utilization_basis": (
                f"Real {best_commodity} tonnage, latest recorded month, annualised vs. berth capacity"
                if best_commodity else "No commodity in this month's dataset maps to this berth"
            ),
        })
    return statuses
